...
export function MiniPlayerProvider({ children }: { children: React.ReactNode }) {
  ...
  const [currentMoodConfig, setCurrentMoodConfig] = useState<MoodConfig | null>(null);
  ...
  const playNext = () => {
    ...
    setPlayingSong({ title: next.title, artist: next.artist, videoId: next.videoId });
    setIsPlaying(true);
  };
  ...
  const playPrev = () => {
    ...
    setPlayingSong({ title: prev.title, artist: prev.artist, videoId: prev.videoId });
    setIsPlaying(true);
  };
